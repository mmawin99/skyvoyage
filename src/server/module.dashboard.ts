...
export const dashboardAdminModule = new Elysia({
  ...
  try {
    ...
    const revenueByRoute = await prisma.$queryRaw<{
        timeInterval: string; 
        origin: string; 
        destination: string; 
        revenue: number;
    ...
    const bookingStatus = await prisma.$queryRaw<{
        status: string;
        count: number;
    }[]>`
      SELECT 
    ...
    const revenuePassengerKilometers = await prisma.$queryRaw<{
        flightId: string; 
        flightNum: string; 
        airlineCode: string; 
        departureTime: Date; 
    ...
    const seatUtilization = await prisma.$queryRaw<{
        flightId: string; 
        flightNum: string; 
        airlineCode: string; 
        departureTime: Date; 
    ...
    const avgTicketPrice = await prisma.$queryRaw<{
        origin: string; 
        destination: string; 
        averagePrice: number;
    }[]>`
    ...
    const passengerDemographics = await prisma.$queryRaw<{
        ageRange: string;
        nationality: string;
        count: number;
    }[]>`
